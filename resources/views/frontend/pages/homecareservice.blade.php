...
@section('title','Home Care Request')
@section('content')
            ...
            <div class="row">
                <div class="col-md-4 offset-md-4">
                    <div class="tele-medicine-box">
                            <h2>@lang('homepage.homecare')</h2>
                            <div class="dot-div mb-4">
                                <span class="dot first-dot"></span>
                            </div>
                            <div class="tele-dedicine-details">
                                   
                                <form action="{{ route('homecareservice.store') }}" method="post" enctype="multipart/form-data">
                                    @csrf

                                        <div class="form-group pb-3">
                                              <label for="">Category <span class="text-danger">*</span></label>
                                              <select name="category_id" id="select_nice2" class="form-control">
                                                    <option value="">-- {{ app()->getLocale() == "en" ?  'Category' : 'المدينة' }} --</option>
                                                    @foreach($homecategories as $category)
                                                        <option  {{ old('category_id') == $category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ app()->getLocale() == "en" ? $category->name : $category->name_ar }}</option>
                                                    @endforeach
                                              </select>
                                        ...
                                        <div class="form-group pb-3">
                                              ...
                                              <select name="service_id" id="select_nice3" class="form-control">
                                                    <option value="">-- {{ app()->getLocale() == "en" ?  'Service' : 'المدينة' }} --</option>
                                                    @foreach($services as $service)
                                                        <option  {{ old('service_id') == $service->id ? 'selected' : '' }} value="{{ $service->id }}">{{ app()->getLocale() == "en" ? $service->name : $service->name_ar }}</option>
                                                    @endforeach
                                              </select>
                                        ...
                                        <div class="form-group pb-3">
                                              ...
                                              <select name="city_id" id="select_nice" class="form-control">
                                                    <option value="">-- {{ app()->getLocale() == "en" ?  'City' : 'المدينة' }} --</option>
                                                    @foreach($cities as $city)
                                                        <option  {{ old('city_id') == $city->id ? 'selected' : '' }} value="{{ $city->id }}">{{ app()->getLocale() == "en" ? $city->name : $city->name_ar }}</option>
                                                    @endforeach
                                              </select>
